...
export async function PATCH(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (eingabe.data.status === 'aktiv') {
    ...
    const result = await resend.emails.send({
      ...
      to: lp.email,
      subject: 'Dein Zeitkapsel-Konto wurde freigeschaltet',
      html: `
        <p>Hallo ${lp.vorname}</p>
        <p>Dein Konto bei Zeitkapsel wurde freigeschaltet. Du kannst dich jetzt anmelden.</p>
        <p>
          <a href="${baseUrl}/anmelden"
    ...
    console.log('[BESTAETIGUNG] Mail-Ergebnis:', JSON.stringify(result))
  }
